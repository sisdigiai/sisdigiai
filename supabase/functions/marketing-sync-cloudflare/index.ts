...
type Supa = any;
type SiteRow = { site: string; cloudflare_zone_id: string | null; sort_order: number };

async function getSites(supabase: Supa, only?: string): Promise<SiteRow[]> {
  let q = supabase.from("v_seo_sites").select("site, cloudflare_zone_id, sort_order");
...
// Erro de configuração: o site não tem zona Cloudflare cadastrada / coberta pelo token.
class ZoneNotConfigured extends Error {
  constructor(site: string) {
    super(`Cloudflare Analytics indisponível para ${site}: cadastre o cloudflare_zone_id em company.seo_sites e garanta que o token cobre essa zona.`);
    this.name = "ZoneNotConfigured";
  }
}

async function syncOneSite(supabase: Supa, token: string, s: SiteRow): Promise<number> {
  const zoneId = s.cloudflare_zone_id;
  if (!zoneId) throw new ZoneNotConfigured(s.site);
  ...
  const r = await fetch(GRAPHQL, {
    ...
    body: JSON.stringify({ query, variables: { zone: zoneId, start: start7, end, start24, end24 } }),
  });
  ...
  for (const g of groups7) {
    ...
    for (const st of (g.sum?.responseStatusMap ?? []) as Array<{ edgeResponseStatus: number; requests: number }>) {
      statusTotals.set(st.edgeResponseStatus, (statusTotals.get(st.edgeResponseStatus) ?? 0) + (st.requests ?? 0));
    }
  }
  ...
  const { error: repErr } = await supabase.rpc("fn_replace_metrics", { p_source: "cloudflare", p_site: s.site, p_rows: rows });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const results: Array<{ site: string; rows_written?: number; error?: string; needs_config?: boolean }> = [];
    for (const s of sites) {
      try {
        const n = await syncOneSite(supabase, token, s);
        results.push({ site: s.site, rows_written: n });
      } catch (e) {
        results.push({ site: s.site, error: String(e), needs_config: e instanceof ZoneNotConfigured });
      }
    }

    // Single-site (botão da aba): zona não configurada vira aviso de "configurar" (amarelo), não erro.
    if (payload.site && results.length === 1 && results[0].needs_config) {
      return jsonResp({ ok: false, configured: false, provider: PROVIDER,
        message: results[0].error, doc: SETUP_DOC, results }, 503);
    }
