...
export function Depoimentos() {
  ...
  return (
    <div className="p-8">
      ...
      {/* Link público */}
      <div className="bg-[#EC4899]/5 border border-[#EC4899]/20 rounded-lg p-4 mb-6">
        <div className="text-[10px] uppercase tracking-widest font-bold text-[#EC4899] mb-2">URL pública pra divulgar</div>
        <div className="flex items-center gap-2">
          <code className="flex-1 text-xs text-white/80 bg-black/30 border border-white/10 rounded px-2 py-1.5 font-mono break-all">{publicUrl}</code>
          <a href={publicUrl} target="_blank" rel="noreferrer" className="text-white/60 hover:text-white p-2"><ExternalLink className="w-4 h-4" /></a>
          <button onClick={copyPublicUrl} className="flex items-center gap-1 text-xs bg-[#EC4899] text-white px-3 py-1.5 rounded hover:bg-[#EC4899]/90 shrink-0">
        ...
        <div className="grid grid-cols-5 gap-3 mb-6">
          {(['pending','approved','used','rejected','spam'] as const).map((s) => (
            <button
              ...
              className={`bg-white/5 border rounded-lg p-3 text-left transition-colors ${statusFilter === s ? 'border-white/30 ring-1 ring-white/20' : 'border-white/10 hover:bg-white/[0.08]'}`}
            >
              <div className="text-[10px] uppercase tracking-widest font-bold text-white/40">{STATUS_LABEL[s].split(' (')[0]}</div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div className="text-xs text-white/40">
          Filtrando: <span className="text-white">{statusFilter === 'all' ? 'todos' : STATUS_LABEL[statusFilter]}</span>
        </div>
      </div>
...
function TestimonialCard({ t, busy, onApprove, onReject, onSpam, onPromote }: {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-lg p-4 border-l-4" style={{ borderLeftColor: STATUS_COLOR[t.status] }}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="font-semibold">{t.full_name}</span>
            {t.optica_name && <span className="text-xs text-white/60">· {t.optica_name}</span>}
            {(t.city || t.state) && <span className="text-xs text-white/40">· {[t.city, t.state].filter(Boolean).join('/')}</span>}
            {t.rating && (
        ...
        <span
          className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded shrink-0"
      ...
      {t.hook_applied && (
        <div className="text-[11px] text-white/50 mb-2">
          <span className="text-white/30">Hook aplicado:</span> {t.hook_applied}
        </div>
      )}

      <p className="text-sm text-white/80 mb-3 whitespace-pre-wrap">{t.story}</p>

      {t.photo_url && (
        <a href={t.photo_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11px] text-[#06B6D4] hover:text-white mb-2">
          <ExternalLink className="w-3 h-3" /> Foto enviada
        </a>
      )}

      {t.promoted_idea_hook && (
        <div className="text-[11px] text-[#06B6D4] mb-2">
          ✓ Já virou ideia: <em>{t.promoted_idea_hook}</em>
        </div>
      )}

      {t.reviewer_notes && (
        <div className="text-[11px] text-white/40 mb-2 italic">
        ...
        {t.status === 'pending' && (
          <>
            <button onClick={onApprove} disabled={busy}
              className="flex items-center gap-1 text-xs bg-[#10B981] text-[#0A0F1E] font-medium px-3 py-1.5 rounded hover:bg-[#10B981]/90 disabled:opacity-50">
              {busy ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Aprovar
            </button>
            <button onClick={onReject} disabled={busy}
              className="flex items-center gap-1 text-xs border border-white/10 text-white/60 px-3 py-1.5 rounded hover:bg-white/5 disabled:opacity-50">
