...
export function Planejador() {
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      {/* Form */}
      <div className="bg-white/5 border border-white/10 rounded-lg p-5 mb-6 space-y-4">
        <h3 className="text-[10px] uppercase tracking-widest font-bold text-white/40">Configuração</h3>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Field label={`Semanas: ${weeks}`}>
            ...
            <div className="text-[10px] text-white/40 mt-1">{phaseLabel(weeks)}</div>
          </Field>
          <Field label={`Posts por semana: ${perWeek}`}>
            <input type="range" min={3} max={7} value={perWeek}
              ...
              className="w-full accent-[#8B5CF6]" />
            <div className="text-[10px] text-white/40 mt-1">{perWeek}×/sem · {totalPosts} posts no total</div>
          </Field>
        </div>
        ...
        <div>
          <div className="text-[10px] uppercase tracking-widest font-bold text-white/40 mb-2">
            ...
            <span className="text-white/30 normal-case ml-2">— cada post vai pra todos esses canais</span>
          </div>
              ...
              return (
                <label key={p.code}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer text-xs
                    ${checked ? 'border-[#8B5CF6]/50 bg-[#8B5CF6]/10' : 'border-white/10 hover:border-white/20'}`}>
      ...
      {!canSchedule && scheduledStats && totalPosts > 0 && (
        <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/30 rounded-lg p-3 mb-4 text-xs text-[#F59E0B] flex items-center gap-2">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={handlePreview} disabled={busy || selectedChannels.length === 0}
          className="flex items-center gap-2 px-4 py-2 border border-white/10 text-sm rounded-lg hover:bg-white/5 disabled:opacity-50">
        ...
        <button onClick={handleSchedule} disabled={busy || !preview || selectedChannels.length === 0}
          className="flex items-center gap-2 px-4 py-2 bg-[#8B5CF6] text-white font-medium rounded-lg hover:bg-[#8B5CF6]/90 text-sm disabled:opacity-50">
        ...
        {(scheduledStats?.planned ?? 0) > 0 && (
          <button onClick={handleClear} disabled={busy}
            className="flex items-center gap-2 px-3 py-2 border border-red-500/30 text-red-300 text-sm rounded-lg hover:bg-red-500/10 disabled:opacity-50">
      ...
      {confirmation && (
        <div className="bg-[#10B981]/10 border border-[#10B981]/30 rounded-lg p-3 mb-6 text-sm text-[#10B981] flex items-center gap-2">
      ...
      {preview && (
        <div className="bg-white/5 border border-white/10 rounded-lg p-5">
          ...
          <div className="mb-4">
            <div className="text-[10px] uppercase tracking-widest font-bold text-white/40 mb-2">Distribuição por pilar</div>
            <div className="flex flex-wrap gap-2">
              {Object.entries(preview.by_pillar).map(([code, count]) => (
                <span key={code} className="text-xs bg-white/5 border border-white/10 rounded px-2 py-1">
                  <b>{code}</b> · {count}
                </span>
              ))}
            </div>
          ...
          <div className="max-h-96 overflow-y-auto space-y-1">
            {(preview.preview ?? []).slice(0, 30).map((p, i) => (
              <div key={i} className="flex items-center gap-3 text-xs py-1 px-2 hover:bg-white/5 rounded">
                <span className="text-white/40 w-20 font-mono">{p.date}</span>
                <span className="bg-white/5 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold text-white/60 w-20 text-center">{p.content_type}</span>
                <span className="flex-1 text-white/80 truncate">{p.hook}</span>
              </div>
            ))}
          </div>
        </div>
...
const inputCls = 'w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#8B5CF6]/50 text-white';

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="text-[11px] text-white/50 block mb-1.5">{label}</label>
      {children}
    </div>
  );
}
