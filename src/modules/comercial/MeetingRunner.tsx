...
export default function MeetingRunner({
  ...
  const save = async () => {
    ...
    await meetingStore.log(session);
    setSaving(false);
    onSaved();
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-surface-low border border-outline/20 w-full max-w-6xl h-[92vh] flex flex-col">
        ...
        <div className="flex-1 flex min-h-0">
          ...
          {/* Captura */}
          <div className="w-[360px] shrink-0 overflow-y-auto p-5 space-y-4 bg-surface-low/60">
                ...
                <div className="mt-1.5 space-y-1">
                  {playbook!.objections.map((o, i) => (
                    <label key={i} className="flex items-start gap-2 text-xs text-on-surface-variant cursor-pointer">
                      <input type="checkbox" checked={objections.includes(o.objecao)} onChange={() => toggleObjection(o.objecao)} className="mt-0.5 accent-current" />
            ...
            {lead && <p className="text-[10px] text-muted text-center">Atualiza o estágio e o próximo passo do lead no pipeline.</p>}
          </div>
        </div>
      </div>
    </div>
