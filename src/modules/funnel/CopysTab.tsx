    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white/3 border border-white/8 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-white/60">{stats.pendente}</div>
          <div className="text-xs text-white/30 mt-1">Pendentes</div>
        </div>
        <div className="bg-[#06B6D4]/5 border border-[#06B6D4]/20 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-[#06B6D4]">{stats.criado}</div>
          <div className="text-xs text-[#06B6D4]/60 mt-1">Criados</div>
        </div>
      ...
      {CATEGORY_ORDER.map((cat) => {
        ...
        return (
          <section key={cat} className="bg-white/3 border border-white/8 rounded-2xl overflow-hidden">
            <button
            ...
            >
              ...
              <ChevronDown
                size={16}
                className={`text-white/30 transition-transform ${isCollapsed ? '' : 'rotate-180'}`}
              />
            </button>
