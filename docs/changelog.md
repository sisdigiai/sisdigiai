...
- **Migration `041_social_updates.sql` (escrita, ⚠️ NÃO aplicada)** — `marketing.social_updates` (log temporal: post/perfil/bio/capa/config/campanha por conta) + view `v_marketing_social_updates` + RPC `marketing_log_social_update` (authenticated; anon revogado). **Bloqueio:** `SUPABASE_TOKEN` (Management API) está 401 — renovar PAT e aplicar. A aba mostra banner âmbar com a instrução até lá; cadastro funciona normalmente. Store: `listSocialUpdates`/`logSocialUpdate` em `marketingStore.ts`.
