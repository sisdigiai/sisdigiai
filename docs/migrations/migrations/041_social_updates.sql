...
-- ✅ Aplicada em 2026-06-10 via Management API (fora do ledger schema_migrations, como as demais).
...
-- security_invoker + revoke anon: padrão da migration 035 (default privileges do
-- Supabase concedem SELECT a anon em view nova — sem isto o log vazaria público)
ALTER VIEW public.v_marketing_social_updates SET (security_invoker = true);
REVOKE ALL ON public.v_marketing_social_updates FROM anon, public;
GRANT SELECT ON public.v_marketing_social_updates TO authenticated;
GRANT USAGE ON SCHEMA marketing TO authenticated;
GRANT SELECT ON marketing.social_updates TO authenticated;
